...
<div class="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-lg p-8">
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-5">
            ...
            <div>
                ...
                <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500 @error('name') border-red-400 @enderror">
                @error('name')
            ...
            <div>
                ...
                <input id="phone" type="tel" name="phone" value="{{ old('phone') }}" placeholder="+880 1700-000000"
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500 @error('phone') border-red-400 @enderror">
                @error('phone')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <p class="text-center text-gray-500 mt-6 text-sm">
            ...
            <a href="{{ route('login') }}" class="text-rose-600 hover:text-rose-700 font-medium">
                {{ __('general.login') }}
            </a>
        </p>
    </div>
