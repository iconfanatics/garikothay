        ...
        <div class="gk-nav-container">
            <div class="flex items-center gap-1 overflow-x-auto">
                @foreach($topMenuItems as $item)
                    <a href="{{ $item['href'] }}"
                        class="shrink-0 rounded-md px-3 py-3 text-sm font-medium transition {{ request()->url() == $item['href'] ? 'is-active' : '' }}">
                        {{ $item['label'] }}
                    </a>
                @endforeach
            </div>
    ...
    <div x-show="mobileOpen" x-cloak class="md:hidden relative z-[100]">
        ...
        <div x-show="mobileOpen"
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
                <div class="space-y-1">
                    <a href="{{ route('home') }}"
                        class="flex items-center gap-3 px-3 py-3 text-gray-700 hover:bg-red-50 hover:text-[var(--gk-red)] rounded-lg text-sm font-medium transition {{ request()->routeIs('home') ? 'text-[var(--gk-red)] bg-red-50' : '' }}">
                        <svg class="w-5 h-5 {{ request()->routeIs('home') ? 'text-[var(--gk-red)]' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                        {{ __('general.home') }}
                    </a>
